...
conda run -n cadt python scripts/train_task_level.py \
...
conda run -n cadt python scripts/random_baseline_task_level.py \
  --config configs/task_level.json \
...
conda run -n cadt python scripts/build_task_embedding_report.py \
