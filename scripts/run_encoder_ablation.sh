...
for i in "${!ENCODERS[@]}"; do
  ...
  run_dir=$(ls -dt "$RUNROOT"/${exp}_* | head -n1)
  if [[ -z "$run_dir" ]]; then
  ...
  report_path="$RUNROOT/encoder_${enc}_report_$(date +%Y%m%d_%H%M%S).xlsx"
...
all_report_path="$RUNROOT/encoder_comparison_$(date +%Y%m%d_%H%M%S).xlsx"
